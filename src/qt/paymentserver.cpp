...
const int GECCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString GECCOIN_IPC_PREFIX("gecicoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(GECCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(GECCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(GECCOIN_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start gecicoin: click-to-pay handler");
    else
