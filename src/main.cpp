...
uint256 hashGenesisBlock("0x00000271316390a2ef0b6d6854c52c2ec3eb3d23524d67d78b538df224535991");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // gecicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "gecicoin Signed Message:\n";
...
{
    // gecicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nSubsidyGenesisBlock = 50 * COIN;
static const int64 nSubsidyInitialBlock = 50 * COIN;
...
static const int nHeightRolloffBlocks = 60;

int64 static GetBlockValue(int nHeight, int64 nFees)
{
    if (nHeight == nHeightGenesisBlock)
    {
        return nSubsidyGenesisBlock;
    }
    ...
    // Subsidy is cut in half every 210240 blocks
    // this is every 12 months
    nSubsidy >>= (nHeight / 210240);

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 150; // gecicoin: 25 minutes
...
{
    // use simple difficulty for first
    if (pindexLast->nHeight+1 < nSubsidyRolloffBlock)
    {
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // gecicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // gecicoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x00000271316390a2ef0b6d6854c52c2ec3eb3d23524d67d78b538df224535991");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        const char* pszTimestamp = "StartUP!!!- Gecicoin Genesis";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = nSubsidyGenesisBlock;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("234560d3723f4cafe4e27d347277f4fc516c5eb7846eb90ae47dccdeb1a440f470") << OP_CHECKSIG;
        ...
        // Merkle root
        assert(block.hashMerkleRoot == uint256("0xdec7cceb714f246c927e2f895355000d4852cfd3dd670dcca23022ed1d2c6a0c"));
...
bool ProcessMessages(CNode* pfrom)
{
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        if (memcmp(msg.hdr.pchMessageStart, pchMessageStart, sizeof(pchMessageStart)) != 0) {
            printf("\n\nPROCESSMESSAGE: INVALID MESSAGEGEC\n\n");
            fOk = false;
            break;
        }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("gecicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("gecicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static gecicoinMiner(CWallet *pwallet)
{
    printf("gecicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("gecicoin-miner");
    ...
    try { loop {
        ...
        printf("Running gecicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("gecicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&gecicoinMiner, pwallet));
}
